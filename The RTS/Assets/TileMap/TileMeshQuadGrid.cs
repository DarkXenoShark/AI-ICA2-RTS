...
namespace BlackTip
{
	public class TileMeshQuadGrid : TileMesh
	{
		public override void SetTile(IVector2 coord, Sprite sprite)
		{
			Texture currentTexture = MaterialTexture;
			...
			SetTile (coord.y * Settings.Tiles.x + coord.x, sprite);
		}
		...
		protected override Mesh CreateMesh()
		{
			...
			int tilesX = Settings.Tiles.x;
			int tilesY = Settings.Tiles.y;
			int quads = tilesX * tilesY; // one quad per tile
			...
			for (int y = 0; y < tilesY; y++)
			{
				...
				{
					int i = y * tilesX + x; // quad index
					int qi = i * 4; // vertex index
					int ti = i * 6;
